...
const NB_USERS = 100;
const NB_TEAMS = 100;
const NB_TOURNAMENTS = 20;
const USERS_PER_TEAM = 4;
const TEAMS_PER_TOURNAMENT = 16;

/**
 * Seeds the database. Safe to call multiple times (idempotent for admin, creates demo data).
 * Does NOT close the database connection (for use when called from server).
 * @param {boolean} closeAfter - If true, closes sequelize after seeding (for CLI usage)
 */
export async function runSeed(closeAfter = false): Promise<void> {
    ...
    function pickScores(result: MatchResult): {
        homeScore: number;
        awayScore: number;
    } {
        ...
        if (result === MatchResult.DRAW)
            return { homeScore: winner, awayScore: winner };
    ...
    for (const tournament of tournaments) {
        ...
        // Ensure we place ALL round-robin matches (extend tournament end if needed).
        const minDaysNeeded = Math.ceil(pairs.length / MAX_PER_DAY);
        ...
        const daysSpan =
            Math.ceil((end.getTime() - day.getTime()) / msPerDay) + 1;
        if (daysSpan < minDaysNeeded) {
            const newEnd = new Date(
                day.getTime() + (minDaysNeeded - 1) * msPerDay
            );
            await tournament.update({ endDate: newEnd });
        }
        ...
        while (pairIdx < pairs.length) {
            ...
            for (let s = 0; s < count; s++) {
                ...
                matchDate.setUTCHours(MATCH_SLOTS[s]!, 0, 0, 0);
...
// CLI: run as standalone script (npm run seed)
